...
- **bulk_read** (única tool HF): para **entender/resumir** cualquier archivo sin traerlo completo al contexto — ahorra 80-90% tokens. Para `> SHUNT_MIN_LINES=350` es obligatorio (el hook bloquea `read`/`cat` sin pipe); para archivos pequeños (`AGENTS.md:122`, `history.md:64`) es **opcional pero recomendado** si tu pregunta cabe en 5-10 bullets. Ej: `bulk_read paths="AGENTS.md" question="¿qué convenciones debo respetar?"` o `bulk_read paths="progress/history.md,AGENTS.md" question="¿qué features faltan vs convenciones?"`. **NO usar si vas a editar**: ahí usa `read offset/limit` puntual.
...
grep -rn "PackCarousel\|PACKS_API" src --include="*.ts" --include="*.tsx" | head
# luego: read src/lib/data/packs.ts offset 1 limit 80
...
wc -l AGENTS.md progress/history.md src/lib/data/packs.ts src/app/api/packs/route.ts

# Bulk voluntario en archivos pequeños (ahorra contexto vs read completo)
# bulk_read paths="AGENTS.md" question="resume stack y convenciones en 8 bullets"
# bulk_read paths="progress/history.md" question="últimos 3 cierres en bullets"
```

Config: `HF_MODEL` en `opencode.json:3` (repo), `HF_API_KEY` en `~/.config/opencode/opencode.jsonc:4` (usuario, no commitear).
