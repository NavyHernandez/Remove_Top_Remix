...
| **Eliminación de Duplicados** (`DuplicateRemovalPage`) | Escanea una carpeta (recursivo, incluye subcarpetas, máx. **1.000 archivos** `AppLimits.DuplicatesMaxFilesToScan`). Pipeline de detección por prioridad: **nombre normalizado → nombre contenido (subconjunto) → hash → palabra clave**. La MISMA CANCIÓN por nombre normalizado (`SameName`) se clasifica como **exacta** y se marca por defecto; también se detectan nombres que difieren en **una sola letra** (falta ortográfica). El detector de **nombre contenido** (`SubsetNameDetector`) agrupa archivos donde todas las palabras del **título** (último bloque) del nombre más corto aparecen en el título del más largo (máx. 3 palabras de diferencia; tope de 6 miembros por cluster). Fix: `StripAllExtensions` elimina extensiones múltiples conocidas (`.mp3.vdjstems` → `.mp3`). Exactos por hash SHA-256 solo sobre los no reclamados por nombre con tamaño repetido (en paralelo). Los "posibles" por palabra clave se verifican por duración de audio. Eliminación con dos opciones: Papelera de Windows (recuperable) o borrado definitivo, ambas con confirmación. Detecta además archivos < **6 KB** (`AppLimits.DuplicatesMinValidFileSizeBytes`) como "dañados" en una 3.ª pestaña. **Previsualizador unificado**: botón en cada fila de las pestañas Exactos/Posibles (módulos `Features/AudioPreview/`, `Features/ImagePreview/` y `Features/DocumentPreview/`, no disponible en dañados) con icono según tipo (`Play` para audio, `Image` para imágenes, `Document` para documentos) que muestra una **tarjeta con forma de onda + scrub** y transporte **Play/Pausa/Stop** para el audio, la **imagen ajustada al espacio** (Stretch Uniform, sin zoom) en la tarjeta de imágenes, o el **texto con scroll** (plano `.txt/.csv/.log/.md` o extraído de Office moderno `.docx/.xlsx/.pptx`, capado a `DuplicatesDocumentPreviewMaxBytes/Lines`) o el **PDF renderizado con WebView2** en la tarjeta de documentos. Un solo preview activo a la vez (abrir uno cierra el otro). Icono check verde cuando no hay duplicados. Botones de acciones centrados. Botón **"Limpiar"** al final de los resultados de eliminación (resetea ruta + resultados). Si el escaneo termina **sin duplicados ni dañados**, el botón principal pasa de "Escanear duplicados" a **"Limpiar"** (flag `_noDuplicates`, ramifica a `ResetAll`) para soltar la carpeta de origen. **Arrastre nativo propio** (sin `DropTargetControl`/`FileSourceControl`): overlay local "Suelta para cargar" (acento `#E74C3C`, timer anti-parpadeo 250 ms); una sola carpeta **carga la ruta sin auto-escanear** (archivos sueltos o varias carpetas muestran aviso). Cada fila de las 3 pestañas tiene **"x"** (`DismissCircle`, `DismissResultItem_Click`) que la quita de la lista sin tocar el disco (cierra el preview si era el archivo mostrado y refresca contadores/resumen/acciones). |
| **Descarga de YouTube** (`DownloaderPage`) | Descarga el mejor audio de un enlace de YouTube y lo normaliza con la cadena de masterización. **Sin binarios en el instalador**: el motor se aprovisiona on-demand en `%LOCALAPPDATA%\Remove_Top\tools` usando **Python embebido firmado por la PSF** + wheel `yt-dlp` de PyPI (código puro, licencia Unlicense, sin el `yt-dlp.exe` de PyInstaller que marca el antivirus) + **deno** (runtime JS para los *challenges*) + **ffmpeg LGPL** (decodifica Opus a WAV); todo con auto-actualización del wheel y borrado del *Mark of the Web*. **Anti-bot**: proveedor de PO tokens **bgutil** (plugin `bgutil-ytdlp-pot-provider` de PyPI + `server/` del mismo release en GitHub, con `deno install` on-demand; arg `youtubepot-bgutilscript:server_home`, caché contenida en `tools/`; el aprovisionamiento **precalienta el script generador** (`WarmUpBgUtilScript`) para que la primera descarga no agote el timeout de 15 s del plugin, y `YtDlpService` **reintenta una vez** ante ese timeout transitorio (`IsPotTimeout`) — las peticiones parecen de un navegador real **sin cuentas ni cookies**; cadena `web,web_embedded,tv,web_safari,mweb` con **cascada de reintento** (`tv,web_safari,mweb` → `android`, pausa 8 s, solo si es bot-check y sin restricción de edad) + `--force-ipv4 --retries 5 --fragment-retries 5 --retry-sleep exp=1:20 --sleep-interval 2` + `--sleep-requests 1` (da Opus 251 ≈133 kbps). `DownloadErrorText` clasifica: no-disponible / región / embed / DRM / bot-check / red. Entrada por URLs (una por línea) o arrastrando un `.txt`; carpeta destino con `FolderPicker`. Un único check arriba **"Normalizar al descargar"** (perfil fijo `HardLimiter` a −1.0 dBFS); si se activa, al terminar **solo queda el archivo normalizado en `OneDj_Normalized`** y se borra el WAV intermedio por la **ruta exacta** del resultado (`NormalizationResult.InputPath`), nunca por nombre. **Popup de error** (patrón de Cuenta: velo + tarjeta con `BounceEase`) al terminar con fallos o si el motor no está listo, con **"Intentar de nuevo"** que reprocesa solo los enlaces fallidos; los mensajes se traducen a texto simple sin códigos (`DownloadErrorText`). Acento azul marino `#19376D` (icono del menú en blanco); botones sólidos con hover blanco unificado. **Máx. 20 enlaces por descarga** (`AppLimits.DownloaderMaxLinksPerBatch`): badge "Versión Gratuita" + aviso en la tarjeta Enlaces, contador `n/20`, bloqueo al agregar (duplicados no consumen cupo) y `Take(20)` defensivo al descargar. **Cuenta de YouTube opcional** (`YouTubeSession` + `YouTubeLoginDialog` con WebView2 embebido a accounts.google.com, perfil aislado, autodetección por cookies, export Netscape a `youtube_cookies.txt`; intento `--cookies` prioritario en `YtDlpService` con fallback anónimo si la sesión muere; tarjeta de 3 estados + sugerencia ante fallos autenticables; OAuth verificado muerto). Checks compactos en una fila: **"Normalizar"** + **"Conservar original"** (apagado por defecto; sin normalizar se deshabilita). **Un solo Limpiar** + botón fantasma **"Abrir ubicación"** (abre en Explorador `OneDj_Normalized` o destino). URLs canonicalizadas a `watch?v=ID` (`YouTubeUrlParser`: fuera listas/radios/tiempos; fix `youtu.be`). Verificación tolerante del archivo (rescate ante tildes/emojis + mensaje `output-not-found`). Popup de error centrado. `DuplicateScanner` excluye carpetas `OneDj_*`. |
| **Cuenta** (`AccountPage`) | Centro de perfil y actualizaciones. La página muestra: logo profesional vectorial (`Assets/BrandLogo.xaml`, gradiente + nota + forma de onda), sección **Perfil** con **autenticación real de Firebase (Email/Password)** — sin sesión muestra un formulario de login/registro; con sesión muestra el perfil y "Cerrar sesión". **Gate de verificación de correo**: solo se admiten cuentas con correo verificado — el login de una cuenta sin verificar NO abre sesión y muestra un panel ámbar con "Reenviar enlace de verificación"; el registro envía el enlace y no deja sesión (panel verde de éxito); el refresh token se conserva como "pendiente" y un polling (`AccountPage`, cada 5 s) detecta la confirmación del correo y hace el **auto-login**. El login de una cuenta inexistente la crea automáticamente y envía el correo de verificación (`AuthService.LoginOrRegisterAsync`). El correo se envía por REST de Identity Toolkit (`FirebaseRestApi`, el paquete v4 no lo expone). La sesión se restaura al iniciar la app. Sección **Sugerencias** (visible SOLO con sesión): cuadro de feedback con tope de 1000 caracteres (contador) que guarda en **Cloud Firestore** vía REST (`FirebaseRestApi.AddSuggestionAsync`, colección `suggestions`). Sección **Actualizaciones** (`UpdateChecker` con **Velopack** — consulta GitHub Releases; badge que se ilumina verde/ámbar; botón "Descargar vX.Y.Z" con ProgressRing de progreso; al llegar a 100% aplica y reinicia; `VelopackApp.Build().Run()` aplica updates pendientes al inicio). **Dot de notificaciones** en el ítem "Cuenta" del menú (`InfoBadge` con `Severity="Attention"`) — se muestra cuando el auto-check al iniciar detecta una versión nueva; se oculta al revisar actualizaciones o al iniciar descarga. Auto-check en background al iniciar (`CheckForUpdatesOnStartup`). Al entrar a Cuenta con update pendiente se eleva un **popup centrado y opaco con rebote** (`BounceEase`, icono `Gift` teal, chip de versión `vX.Y.Z`, "¡Tenemos una nueva actualización!", una sola vez por versión) cuyo botón "Descargar ahora" inicia la descarga directa (`StartDownloadAsync` refactorizado); "Ahora no" o el velo lo cierran (`UpdateChecker.HasPendingUpdate/PendingVersion`). Ítem de menú "Cuenta" con icono `Person` y color teal `#00A88F`. Textos del encabezado centralizados en `AppLimits` (`AccountPageTitle/Subtitle`). Sección **Soporte** al final: tarjeta compacta centrada con QR (`Assets/SupportQr.jpeg`, con placeholder si falta la imagen) y botón teal "Abrir chat de WhatsApp" (`wa.me/593982311600`); estilo glass neutro sobre la base de la app (sombra `ThemeShadow` como recurso + velo superior + línea de luz + acento teal, sin verdes). |
...
└── Remove_Top/
    ...
    └── Remove_Top/
        ...
        │   │   └── PatternInserter.cs             # Inserción de patrón (inicio/centro/final)
        │   ├── VocalRemoval/
        ...
        │   ├── Downloader/
        │   │   ├── DownloaderPage.xaml / .cs    # Descarga de YouTube + normalización encadenada (UI + ViewModel inline)
        │   │   ├── YtDlpService.cs              # Runner de yt-dlp (progreso, centinela, cancelación, anti-bot)
        │   │   ├── ToolManager.cs               # Provisioning on-demand (Python/wheels/deno/ffmpeg + auto-update + MOTW)
        │   │   ├── DownloadModels.cs            # DTOs (DownloadItem/Status/Progress/Result, ToolProgress)
        │   │   ├── DownloadErrorText.cs         # Traducción de errores a texto simple sin códigos
        │   │   └── YouTubeUrlParser.cs          # Validación/normalización de enlaces
│   ├── Account/
...
App.xaml.cs (Application)
  └── MainWindow (NavigationView)
        ...
        ├── DownloaderPage    → YtDlpService + ToolManager (Python/yt-dlp/deno/ffmpeg on-demand) + AudioNormalizer
...
Módulo reutilizable `Features/ImagePreview/` (solo API nativa de WinUI 3, sin dependencias nuevas) que complementa al de audio: el botón de preview de las filas es **unificado** — icono `Play` para audio, `Image` para imágenes, `Document` para documentos (`DuplicateItem.IsAudio` / `IsImage` / `IsDocument` / `IsPreviewable` / `PreviewIcon`). Para tipos no visualizables (video, Office legacy `.doc/.xls/.ppt`, etc.) el botón **no se oculta ni deja hueco**: queda **siempre visible pero deshabilitado** con icono `EyeOff` y tooltip "Sin previsualización" (`DuplicateItem.PreviewToolTip`).
...
## Previsualizador de documentos (detalle)

Módulo reutilizable `Features/DocumentPreview/` (sin dependencias nuevas: OOXML con `System.IO.Compression` + `XmlReader` del propio .NET y PDF con el WebView2 que ya usa el login de YouTube) que completa el botón unificado de las filas — icono `Document` (`DuplicateItem.IsDocument`, sumado a `IsPreviewable` / `PreviewIcon`).

- **`DocumentPreviewSupport`** — `Classify(path)` → `PlainText` (`.txt/.csv/.log/.md`) / `OfficeOpenXml` (`.docx/.xlsx/.pptx`) / `Pdf` (`.pdf`) / `Unsupported` (legacy `.doc/.xls/.ppt`, que quedan con `EyeOff`). `ExtractTextAsync` en hilo de fondo con `FileShare.Read` (no bloquea el borrado): docx → párrafos de `word/document.xml`; xlsx → cadenas compartidas + celdas (tabuladas, 500 filas/hoja); pptx → texto por diapositiva. UTF-8 estricto con fallback a Latin-1 (idéntico a Windows-1252 en el rango del español, sin paquete CodePages). XML con `DtdProcessing.Prohibit`.
- **`DocumentPreviewView`** — estados **vacío / cargando / error** + texto con scroll (Consolas, seleccionable) o `WebView2` para el PDF. `LoadAsync(path, maxBytes, maxLines)`, `Clear()` (texto a vacío + PDF a `about:blank`), `CurrentPath` y eventos `DocumentLoaded(string resumen)` / `DocumentLoadFailed`. El catch es amplio a propósito: un preview nunca tumba la página.
- **Integración**: tarjeta `DocumentPreviewSection` (entre la de imágenes y las acciones) con header (icono `Document` + nombre + cerrar), el visor (texto 300 px / PDF 380 px) y un pie con **líneas + tamaño en disco** ("primeras 3.000 líneas · 210 KB", "PDF · 1,2 MB"). Topes en `AppLimits` (`DuplicatesDocumentPreviewMaxBytes = 256 KB`, `DuplicatesDocumentPreviewMaxLines = 3000`).
- **Un solo preview activo**: `StopAllPreviews()` suma `ClearDocumentPreview()` a los 4 puntos existentes (cambiar preview/carpeta, antes de borrar, `Unloaded`).
...
- **`FileSourceControl`** — tarjeta "Origen" genérica (carpeta/archivos + pista de arrastre + "Limpiar" + tinte al cargar). Configurable: `FileFilter`, `ScanRecursive`, `MaxFiles`, `AccentColor`, `PickerExtensions`, `ShowBrandSite` (marca `www.top-remix.com` centrada en la línea "Origen", opt-in apagado por defecto) y `ShowClearButton` (botón "Limpiar" del origen, visible por defecto; opt-out para páginas con limpieza propia, p. ej. Edición Rápida). Expone `Files/HasFiles/Truncated/TotalFound`, `StateChanged`, `LoadSource/Reset/SetEnabled/SetStatus`.
...
**Colores por feature** (en `MainWindow.xaml` + overlay/tarjetas): Normalizar `#5B9BD5`, Renombrar `#70AD47`, Editar `#E67E22`, Stems `#9B59B6`, Duplicados `#E74C3C`, **Etiquetas `#B8860B` (dorado)**, Cuenta `#00A88F` (teal), **Descargar `#19376D` (azul marino)**.
...
- La fila compacta de límite (badge "Versión Gratuita" + texto en letra 11) se monta en runtime (`NormalizationPage` constructor) usando `AppLimits.NormalizationInfoBarTitle/Message`.
...
## Descarga de YouTube (detalle)

Módulo `Features/Downloader/`. Descarga el mejor audio de YouTube y lo encadena con la Normalización.

- **Motor on-demand, sin binarios en el instalador** (`ToolManager`): Python embebido firmado por la Python Software Foundation + wheels `yt-dlp` y `yt-dlp-ejs` de PyPI (código puro Unlicense; evita el `yt-dlp.exe` de PyInstaller que dispara el antivirus) + `deno` (runtime JS para los challenges) + `ffmpeg` LGPL (decodifica Opus a WAV). Todo en `%LOCALAPPDATA%\Remove_Top\tools`, con **auto-update** comparando la versión de PyPI, extracción **anti zip-slip** (verifica que la ruta quede dentro del destino) y borrado del **MOTW** (`File.Delete(path + ":Zone.Identifier")`).
- **`YtDlpService`**: ejecuta `python.exe -m yt_dlp` como `Process` (`CreateNoWindow`, stdout/stderr redirigidas en UTF-8) con `ArgumentList` (nunca concatenación → evita inyección), `--newline`, y plantilla de salida con **centinela** `after_move:onedj:%(filepath)s` para capturar la ruta final (admite espacios, que era el bug original). `-f bestaudio/best -x --audio-format wav` (Opus → WAV con ffmpeg; cae a muxed si no hay audio-only) con `--ffmpeg-location` a la ruta del exe. `--js-runtimes deno:*` + `--remote-components ejs:github`. Estrategia anti-bot: `--extractor-args "youtube:player_client=web_embedded,tv,web_safari,mweb"` + `--sleep-requests 1` (verificado: `web_embedded` da Opus 251 ≈133 kbps sin cookies; `default` da bot-check). Cancelación mata el árbol de procesos (`Kill(entireProcessTree: true)`); salvaguarda del archivo `.wav`/`.m4a` más reciente si no se imprimió la ruta.
- **Flujo** (`DownloaderPage`, ViewModel inline): Fase 0 `ToolManager.EnsureToolsAsync` (salida limpia si el motor no queda listo) → Fase 1 descarga secuencial por URL (fila con porcentaje en vivo) → Fase 2 `AudioNormalizer.ProcessFilesAsync` con perfil fijo **HardLimiter −1.0 dBFS** → `CorrectOutputNames` → borrado del WAV intermedio **solo si masterizó bien** (si no, se conserva para no perder nada). ProgressText narra cada fase ("Limpiando archivos temporales..."). Sin MasterizeCheckBox activado se conserva la descarga tal cual.
- **`DownloadErrorText`**: traduce los errores crudos de yt-dlp a mensajes simples sin códigos (bot-check, red, no disponible, 403, formato); el detalle técnico se registra en `crash.log` y también se muestra en la fila fallida como texto simple.
- **Popup de error** (`ErrorPopupOverlay`/`ErrorPopupCard` + `ErrorPopupShowStoryboard`): mismo patrón que Cuenta (velo `#66000000` que cierra al tocar + tarjeta que sube con `BounceEase`, borde del acento, icono `Warning` ámbar). Se eleva al terminar con fallos o si el motor no está listo; **"Intentar de nuevo"** (`ErrorPopupRetryButton`, sólido con el acento) reprocesa solo los enlaces en estado `Error` (refactor: `RunFlowAsync` compartido); "Cerrar" lo descarta.
- **UI**: acento azul marino `#19376D` centralizado en `Page.Resources` (`DownloaderAccentBrush/Soft/Light`); botón "Pegar" eliminado; botones de acción sólidos unificados (fondo marino, letras blancas) con hover blanco global (`Button*PointerOver/Pressed` en recursos); item de menú "Descargar" con icono `ArrowDownload` en blanco (el azul no se veía); check "Normalizar al descargar" arriba; subtítulo reducido (12 px) que indica la subcarpeta de salida; InfoBar permanente eliminado (queda como aviso contextual solo ante advertencias/errores).
...
| Descarga YouTube | `DownloaderMaxLinksPerBatch` | 20 |

Las funcionalidades **consumen la lógica y los textos de UI desde `AppLimits`**: los servicios los usan en sus `Take(n)`/topes reales y las páginas montan los avisos de límite, contadores y descripciones en runtime desde las propiedades de texto (`AppLimits.NormalizationInfoBar*`, `DuplicatesInfoBar*`, `BatchRenameLimitMessage`, `VocalRemovalPageDescription`). Así los textos nunca se desincronizan de los límites reales. Para cambiar un límite, editar el valor aquí y recompilar.
...
3. **Limpieza local de `releases/`**: borra los `.nupkg` (full y delta) de versiones anteriores — en `releases/` **SOLO queda la última versión**. Va DESPUÉS del pack a propósito (el delta se calcula contra el full anterior); no se tocan `Setup.exe`, `Portable.zip`, `RELEASES` ni manifiestos.
4. **Limpieza idempotente**: borra cualquier release/tag `v$Version` existente (incluidos drafts huérfanos) vía REST API antes de subir. Busca la release por **tag_name o por nombre** (vpk rechaza con "There is already an existing release named" cuando coincide el nombre aunque el tag sea distinto) y borra el tag en ambos formatos (`vX.Y.Z` y `X.Y.Z`). Así re-ejecutar con la misma versión nunca falla con "already exists".
5. `vpk upload github` (sube a GitHub Releases con token), mostrando la salida real de vpk. Se pasa **`--publish`** (sin ese flag vpk deja la release como **borrador** y el update no llega a los usuarios).
...
4. **Verificar** que `releases\` contenga SOLO los archivos de la nueva versión (los `.nupkg` antiguos se borran automáticamente en el paso 3 del script) y que el Setup.exe tenga fecha actual.
...
3. **Empaquetar** — ejecutar `.\publish.ps1 -SkipUpload` desde la raíz y verificar que `releases\OneDjApp-win-Setup.exe` tenga fecha actual (los artefactos de `releases/` están gitignoreados y el script deja solo la última versión; el Release de GitHub lo publica el CI).
